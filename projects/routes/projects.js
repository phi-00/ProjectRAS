...
router.post("/:user/:project/process/cancel", (req, res, next) => {
  // Cancel all ongoing processes for this project and delete their results
  Process.getProject(req.params.user, req.params.project)
    .then(async (processes) => {
      try {
        // Delete all ongoing processes
        for (let p of processes) {
          await Process.delete(req.params.user, req.params.project, p._id);
        }
    ...
    .catch((_) =>
      res.status(501).jsonp(`Error acquiring user's project processes`)
    );
});
